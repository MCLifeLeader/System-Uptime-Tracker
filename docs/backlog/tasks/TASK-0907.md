---
id: TASK-0907
title: 'Preserve the prior binary path and release until startup validation succeeds'
type: task
status: not-started
epic: EPIC-09
release_gate: 'Gate 2'
...
# TASK-0907: Preserve the prior binary path and release until startup validation succeeds

## Objective

Preserve the prior binary path and release until startup validation succeeds; automatically restore and restart it on failed upgrade.
...
- Preserve the prior binary path and release until startup validation succeeds.
- Automatically restore and restart it on failed upgrade.
...
- Deliberately broken release proves rollback to the previously healthy version.
