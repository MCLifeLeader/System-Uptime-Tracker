---
id: TASK-0304
title: 'Store server-authoritative `ReceivedAtUtc`'
type: task
status: not-started
epic: EPIC-03
release_gate: 'Gate 1'
...
# TASK-0304: Store server-authoritative `ReceivedAtUtc`

## Objective

Store server-authoritative `ReceivedAtUtc`; preserve client `SentAtUtc` separately and define precision consistently.
...
- Store server-authoritative `ReceivedAtUtc`.
- Preserve client `SentAtUtc` separately and define precision consistently.
...
- Integration test proves a client cannot set `ReceivedAtUtc`.
