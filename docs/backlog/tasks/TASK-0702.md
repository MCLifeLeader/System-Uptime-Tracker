---
id: TASK-0702
title: 'Define the durable local identity-state boundary and implement atomic first-run `AgentId` creation and load with corrupt-file handling and an OS-supplied durable-state path'
type: task
status: not-started
epic: EPIC-07
release_gate: 'Gate 1'
depends_on: ['TASK-0701']
unblocks: ['TASK-0706', 'TASK-0707']
---

# TASK-0702: Define the durable local identity-state boundary and implement atomic first-run `AgentId` creation and load with corrupt-file handling and an OS-supplied durable-state path

## Objective

Define the durable local identity-state boundary and implement atomic first-run `AgentId` creation and load with corrupt-file handling and an OS-supplied durable-state path.
...
- Parallel starts converge on one ID; restart preserves it; corruption produces an actionable failure; secret-bearing state is delegated to protected storage.
