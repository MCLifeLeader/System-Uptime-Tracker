---
id: TASK-0702
title: 'Implement atomic first-run `AgentId` creation and load with corrupt-file handling and an OS-supplied durable-state path'
type: task
status: not-started
epic: EPIC-07
release_gate: 'Gate 1'
...
# TASK-0702: Implement atomic first-run `AgentId` creation and load with corrupt-file handling and an OS-supplied durable-state path

## Objective

Implement atomic first-run `AgentId` creation and load with corrupt-file handling and an OS-supplied durable-state path.
...
- Parallel starts converge on one ID; restart preserves it; corruption produces an actionable failure.
