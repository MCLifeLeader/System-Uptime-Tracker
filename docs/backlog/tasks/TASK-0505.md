---
id: TASK-0505
title: 'Update machine `FirstSeenAtUtc`, `LastSeenAtUtc`, metadata, and registration state using server-authoritative rules'
type: task
status: not-started
epic: EPIC-05
release_gate: 'Gate 1'
depends_on: ['TASK-0502']
unblocks: ['TASK-0507', 'TASK-1104']
---

# TASK-0505: Update machine `FirstSeenAtUtc`, `LastSeenAtUtc`, metadata, and registration state using server-authoritative rules

## Objective

Update machine `FirstSeenAtUtc`, `LastSeenAtUtc`, metadata, and registration state using server-authoritative rules.
...
- Out-of-order queued heartbeats cannot move `LastSeenAtUtc` backward or overwrite newer metadata incorrectly.
