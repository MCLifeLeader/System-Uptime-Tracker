---
id: TASK-1002
title: 'Finalize Linux service name, executable path, release path, state path, configuration path, and unprivileged user/group under `SystemUptimeTracker` naming'
type: task
status: not-started
epic: EPIC-10
release_gate: 'Gate 2'
...
# TASK-1002: Finalize Linux service name, executable path, release path, state path, configuration path, and unprivileged user/group under `SystemUptimeTracker` naming

## Objective

Finalize Linux service name, executable path, release path, state path, configuration path, and unprivileged user/group under `SystemUptimeTracker` naming.
...
- Architecture and runbook contain no legacy `ComputerTelemetry` path or unit names.
