---
id: TASK-0303
title: 'Add unique indexes for populated `AgentId` and heartbeat idempotency, plus indexes for machine/time and session queries'
type: task
status: not-started
epic: EPIC-03
release_gate: 'Gate 1'
...
# TASK-0303: Add unique indexes for populated `AgentId` and heartbeat idempotency, plus indexes for machine/time and session queries

## Objective

Add unique indexes for populated `AgentId` and heartbeat idempotency, plus indexes for machine/time and session queries.
...
- Duplicate agent and sequence writes fail at the database boundary; query indexes appear in the migration.
