---
id: TASK-0904
title: 'Stage immutable versioned releases under `Program Files`, keep durable state under `ProgramData`, and prevent source/application/data path overlap'
type: task
status: not-started
epic: EPIC-09
release_gate: 'Gate 2'
depends_on: ['TASK-0903']
unblocks: ['TASK-0905', 'TASK-0908']
---

# TASK-0904: Stage immutable versioned releases under `Program Files`, keep durable state under `ProgramData`, and prevent source/application/data path overlap

## Objective

Stage immutable versioned releases under `Program Files`, keep durable state under `ProgramData`, and prevent source/application/data path overlap.
...
- [TASK-0903](./TASK-0903.md): Implement `Install-SystemUptimeTrackerWindowsService.ps1` as an advanced, elevation-checked, parameter-validated, `SupportsShouldProcess` script using its own artifact directory
...
- Repeat install does not replace durable identity or queue files.
